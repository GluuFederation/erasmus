...
Badge Manager retrieve participants from FIDES based on city and state provided from user from mobile app and returns participants list to mobile app.
...
### (3) Making badge request
Badge Manager exposes an API to mobile app using which user can select template badge and can request that particular badge.

Badge manager persist badge request in LDAP for that user.
...
```json
{
  "opHost": "string",
  "participant": "string",
  "templateBadgeId": "string",
  "templateBadgeTitle": "string"
}
```
...
Badge manager exposes an API to FIDES to return badge requests (approved or pending) for particular participant.
...
Badge manager exposes an API to FIDES to approve badge request. 

In this API, Badge request get approved and badge entry get persisted in LDAP as soon as badge request get approved. 
...
Badge manager exposes an API to mobile app which returns approved and pending badge requests which get displayed in mobile app.
...
```json
{
  ...
  "badgeRequests": [
    ...
    {
      "inum": "@!4301.2A50.9A09.7688!1002!3EDE.FA25",
      "participant": "5923f1de8bc19a19d7e939c3",
...
### (7) Delete badge request
Badge Manager exposes an API to the mobile app which user can use to delete badge request.
...
### (8) Set privacy for approved badge request    
Badge manager exposes an API to the mobile app for the user to set badge privacy to `public` or `private`.

By default badge privacy will be public which user can changes through API for approved badge requests.

User can set badge privacy only to approved badge requests because badge entry get persisted in LDAP only after badge request get approved.
...
Badge manager generates badge QR code and return basic badge info along with badge QR code url and user info.

This API will be used by Asserter to present badge to verifier in mobile app.
...
### (10) Verify badge and provide actual badge information
Badge manager exposes an API to mobile app to verify badge and this API will be used by `verifier` in mobile app.

At the time of badge QR code image creation, temporary link of the badge which is an endpoint for retrieving actual badge information get generated which has expiration (90 seconds for now) and stored as a data in QR code.
...
`Note: Temporary link which is store in badge QR code image will get expire in 90 seconds.`
